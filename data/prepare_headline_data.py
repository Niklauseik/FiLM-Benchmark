...
def fetch_dataset(split, offset, length=100):
    url = f"https://datasets-server.huggingface.co/rows?dataset=TheFinAI/flare-headlines&config=default&split={split}&offset={offset}&length={length}"
    response = requests.get(url)
    response.raise_for_status()  # Ensure we notice bad responses
    return response.json()['rows']

def prepare_headline_data(data_path):
