...
def fetch_dataset(offset, length=100):
    url = f"https://datasets-server.huggingface.co/rows?dataset=TheFinAI/flare-mlesg&config=default&split=test&offset={offset}&length={length}"
    response = requests.get(url)
    response.raise_for_status()  # Ensure we notice bad responses
    return response.json()['rows']

def prepare_esg_cls_data(data_path):
