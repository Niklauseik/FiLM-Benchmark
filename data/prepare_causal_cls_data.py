...
def fetch_dataset(offset, length=100):
    url = f"https://datasets-server.huggingface.co/rows?dataset=ChanceFocus/flare-causal20-sc&config=default&split=test&offset={offset}&length={length}"
    response = requests.get(url)
    response.raise_for_status()  # Ensure we notice bad responses
    return response.json()['rows']

def prepare_causal_cls_data(data_path):
