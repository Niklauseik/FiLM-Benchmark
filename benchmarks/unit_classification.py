...
class UnitClassificationDataset(Dataset):
...
def preprocess_data(file_path, tokenizer, max_length, save_path=None):
    ...
    dataset = UnitClassificationDataset(encodings, labels)
    return dataset

def fine_tune_model(model_name, data_path, max_length=128, batch_size=32, epochs=12):
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    
    flag, train_dataset = get_or_preprocess_data(data_path, UnitClassificationDataset, "train")
    if not flag:
        train_dataset = preprocess_data(f"data/raw/{data_path}/train.csv", tokenizer, max_length, f"data/processed/{data_path}/train.pt")
        
    flag, val_dataset = get_or_preprocess_data(data_path, UnitClassificationDataset, "valid")
    if not flag:
    ...
    save_model(model, tokenizer, f'./models/unit_classification/{model_name}')
...
def test_unit_classification(model_name, data_path, max_length=128, batch_size=32):
    model_dir = f'./models/unit_classification/{model_name}'
    ...
    flag, test_dataset = get_or_preprocess_data(data_path, UnitClassificationDataset, "test")
    if not flag:
