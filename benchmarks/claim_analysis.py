...
class ClaimAnalysisDataset(Dataset):
...
def fine_tune_model(model_name, data_path, max_length=128, batch_size=32, epochs=3):
    ...
    save_model(model, tokenizer, f'./models/claim_analysis/{model_name}')
    
    shutil.rmtree(output_dir)

    return model, tokenizer

def test_claim_analysis(model_name, data_path, max_length=128, batch_size=32):
    model_dir = f'./models/claim_analysis/{model_name}'
    ...
    flag, test_dataset = get_or_preprocess_data(data_path, ClaimAnalysisDataset, "test")
    if not flag:
