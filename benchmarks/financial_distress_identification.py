...
def preprocess_data(file_path, tokenizer, max_length, save_path=None):
    ...
    dataset = FinancialDistressDataset(encodings, labels)
    return dataset

def fine_tune_model(model_name, data_path, max_length=128, batch_size=32, epochs=3):
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    
    flag, train_dataset = get_or_preprocess_data(data_path, FinancialDistressDataset, "train")
    if not flag:
        train_dataset = preprocess_data(f"data/raw/{data_path}/train.csv", tokenizer, max_length, f"data/processed/{data_path}/train.pt")
        
    flag, val_dataset = get_or_preprocess_data(data_path, FinancialDistressDataset, "validation")
    if not flag:
    ...
    save_model(model, tokenizer, f'./models/financial_distress_identification/{model_name}')
    
    shutil.rmtree(output_dir)

    return model, tokenizer

def test_financial_distress_identification(model_name, data_path, max_length=128, batch_size=32):
    model_dir = f'./models/financial_distress_identification/{model_name}'
    ...
    flag, test_dataset = get_or_preprocess_data(data_path, FinancialDistressDataset, "test")
    if not flag:
